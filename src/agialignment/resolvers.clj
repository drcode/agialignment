  ...
  (let [{:keys [session-response]} request]
    (when oauthToken
      (when-let [{:keys [screen_name]
                  :as   response} #_{:oauth_token "35367022-7EZgRVQYZ1JGX2qvqGzaHkSgLrFQ8a0TRLuBzKsBb", :oauth_token_secret "Es8XuFJHT4cuFfYseSjuG5Vz1U8RedszfIOZ1Q2MkUBB6", :user_id "35367022", :screen_name "lisperati"}
                 (tw/signup-response oauthToken oauthVerifier)]
...
(defn delete-avatar [db
  ...
  (when-let [userid (adjusted-userid context args)]
    ...
    (let [{:keys [session-response]} request]
      (reset! session-response {:userid nil})))
  {:id     "app:0"
   :userid (session-userid request)})
